...
class RetinaNet(nn.Module):
    ...
    The input to the models is expected to be a list of tensors, each of shape [C, H, W], one for each
    image, and should be in 0-1 range. Different images can have different sizes.

    The behavior of the models changes depending if it is in training or evaluation mode.

    During training, the models expects both the input tensors, as well as a targets (list of dictionary),
    containing:
        - boxes (``FloatTensor[N, 4]``): the ground-truth boxes in ``[x1, y1, x2, y2]`` format, with
          ``0 <= x1 < x2 <= W`` and ``0 <= y1 < y2 <= H``.
        - labels (Int64Tensor[N]): the class label for each ground-truth box

    The models returns a Dict[Tensor] during training, containing the classification and regression
    losses.

    During inference, the models requires only the input tensors, and returns the post-processed
    ...
    Example:
        ...
        >>> from torchvision.models.detection.anchor_utils import AnchorGenerator
        >>> # load a pre-trained models for classification and return
        >>> # only the features
        ...
        >>> )
        >>>
        >>> # put the pieces together inside a RetinaNet models
        >>> models = RetinaNet(backbone,
        >>>                   num_classes=2,
...
def retinanet_resnet50_fpn(pretrained=False, progress=True,
    ...
    """
    Constructs a RetinaNet models with a ResNet-50-FPN backbone.
    ...
    Args:
        pretrained (bool): If True, returns a models pre-trained on COCO train2017
        progress (bool): If True, displays a progress bar of the download to stderr
        num_classes (int): number of output classes of the models (including the background)
        pretrained_backbone (bool): If True, returns a models with backbone pre-trained on Imagenet
