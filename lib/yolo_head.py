...
class YOLOLayer(nn.Module):
    ...
    def __init__(self, anchors, num_classes):
        ...
        anchors = torch.tensor(list(chain(*anchors)))\
            .float().view(-1, 2)
...
class YoloHead(nn.Module):
    def __init__(self, input_channels: list,
        ...
        self.fpn_yolo = FeaturePyramidNetwork(
            in_channels_list=input_channels,
            out_channels=21)
        self.yolo_layers = nn.ModuleList()
        self.yolo_layers.append(
            YOLOLayer(anchors=[(116, 90), (156, 198), (373, 326)], num_classes=2))
        self.yolo_layers.append(
            YOLOLayer(anchors=[(30, 61), (62, 45), (59, 119)], num_classes=2))
        self.yolo_layers.append(
            YOLOLayer(anchors=[(10, 13), (16, 30), (33, 23)], num_classes=2))
    ...
    def forward(self, features: list):
        feets_yolo = self.fpn_yolo(features)
        yolo_outputs = [self.yolo_layers[i](
            feets_yolo[str(i)], 480) for i in range(3)]
        outputs = {
            "det": yolo_outputs if self.training else torch.cat(yolo_outputs, 1)}
        if self.with_uncertainty:
            outputs["det_uncertainty"] = [torch.mean(
                self.uncertainty_layer(feets_yolo["0"]))]
        return outputs
    ...
    def compute_loss(self, predictions, targets):
        ...
        for layer_index, layer_predictions in enumerate(predictions):
            ...
            tobj = torch.zeros_like(
                layer_predictions[..., 0], device=device)  # target obj
            ...
            if num_targets:
                ...
                iou = bbox_iou(
                    pbox.T, tbox[layer_index], x1y1x2y2=False, CIoU=True)
    ...
    def build_targets(self, predictions, targets):
        ...
        # number of anchors, targets #TODO
        num_anchors, num_targets = 3, targets.shape[0]
        tcls, tbox, indices, anch = [], [], [], []
        # normalized to gridspace gain
        gain = torch.ones(7, device=targets.device)
        # Make a tensor that iterates 0-2 for 3 anchors and repeat that as many times as we have target boxes
        anchor_index = torch.arange(num_anchors,
                                    device=targets.device).float().view(num_anchors, 1).repeat(1, num_targets)

        # Copy target boxes anchor size times and append an anchor index to each copy the anchor index is also expressed by the new first dimension
        targets = torch.cat(
            (targets.repeat(num_anchors, 1, 1), anchor_index[:, :, None]), 2)
        ...
        for i, yolo_layer in enumerate(self.yolo_layers):
            ...
            gain[2:6] = torch.tensor(predictions[i].shape)[
                [3, 2, 3, 2]]  # xyxy gain
            ...
            indices.append(
                (b, a, gj.clamp_(0, (gain[3] - 1).long()), gi.clamp_(0, (gain[2] - 1).long())))
