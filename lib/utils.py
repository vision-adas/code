...
def provide_determinism(seed=42, save_path=None):
    # 为了保证实验的可重复性，设置随机种子
    random.seed(seed)
    np.random.seed(seed)
...
def log_everything(logger: SummaryWriter, log_var: dict, iteration):
    # 将所有需要记录的变量记录到tensorboard
    for key in log_var.keys():
        value = log_var[key]
        logger.add_scalar(str(key), value, global_step=iteration)


def convert_to_device(data: Dict, device):
    # 将dict数据转移到指定的设备上
    for k, v in data.items():
        if 'kp' not in k:
            data[k] = torch.as_tensor(v).float().to(device)
    return data
        ...
        if len(annotations):
            ...
            for pred_i, (pred_box, pred_label) in enumerate(zip(pred_boxes, pred_labels)):
                ...
                iou, box_filtered_index = bbox_iou(
                    pred_box.unsqueeze(0), torch.stack(filtered_targets)).max(0)
