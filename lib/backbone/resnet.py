...
class BasicBlock(nn.Module):
    ...
    def __init__(
        ...
        self.with_mask = True
        if self.with_mask:
            self.channel_mask = nn.Parameter(torch.rand(planes, dtype=torch.float32), requires_grad=True)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.with_mask:
            # channel_mask 64
            mask = F.relu6(self.channel_mask)
            # 32x64
            mask = torch.repeat_interleave(mask.unsqueeze(0), torch.tensor(out.size(0)).to(mask.device), 0)
            # 32x64x1x1
            mask = mask.view((out.size(0), out.size(1), 1, 1))
            # out 32x64x120x160
            out = out*mask
